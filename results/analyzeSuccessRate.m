...
% processedData = readtable('processedData20220408.csv');
processedData = readtable('processedData20211019.csv');
...
% indexListNoise  = allIndices(strcmpi(processedData.whichParam, {'NoiseDegree'}) == 1 & processedData.NoiseDegree == 0.1 & processedData.AlignWeight == 10 & processedData.flockingEscape == 0);

% indexListNoise  = allIndices(strcmpi(processedData.whichParam, {'NumAgents'}) == 1 & processedData.NumAgents == 3 & processedData.AlignWeight == 10 & processedData.flockingEscape == 1);

% indexListNoise  = allIndices(strcmpi(processedData.whichParam, {'TurnRate'}) == 1 & processedData.TurnRate == 0.1);

pairedList = [setNum(indexListNoise); repNum(indexListNoise)]';
...
for currRow = 1:size(pairedList,1)
    figure(2)%processedData.ObstacleType(indexList(currRow))
    hold on
    ...
    xList = agentStatesList(1:numberOfAgents,:) + currRow*30;
